...
import { defaultSite, type CreateLeadResponse, type PublicSite } from '@/lib/site-data';
...
export function ContactPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await apiFetch<CreateLeadResponse>('/api/public/leads', { method: 'POST', body: JSON.stringify({ name: data.get('name'), whatsapp: data.get('whatsapp'), email: data.get('email'), legalArea: data.get('legalArea'), consultationType: data.get('consultationType'), preferredDate: data.get('preferredDate') || null, message: data.get('message'), source: 'Formulario contacto' }) });
  ...
  return (
    <PageFrame active="/contacto" profile={site.profile} site={site}>
