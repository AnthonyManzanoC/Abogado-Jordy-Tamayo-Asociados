...
export function EditorialShowcase({ posts }: { posts: MediaPost[] }) {
  ...
  if (!post) return null;
  const src = tiktok ? `${embed}&muted=1&description=1&music_info=1&rel=0`.replace('autoplay=0', 'autoplay=1') : embed;
  return <div ref={section} className="mt-12 overflow-hidden rounded-[2rem] border border-border bg-card lg:grid lg:grid-cols-[.85fr_1.15fr]">
    <div className="bg-[#151410] p-5 sm:p-8">
      ...
      <div className="relative mx-auto aspect-[9/16] w-full max-w-[350px] overflow-hidden rounded-2xl bg-black">
      ...
      <div className="mx-auto mt-5 flex max-w-[350px] items-center justify-between gap-3 text-xs text-white/65"><button onClick={() => setPaused(!paused)} aria-pressed={paused} className="inline-flex items-center gap-2 rounded-full border border-white/20 px-4 py-2">{paused ? <Play className="size-3" /> : <Pause className="size-3" />}{paused ? 'Reanudar' : 'Pausar'}</button><span>Automático · sin sonido</span></div>
