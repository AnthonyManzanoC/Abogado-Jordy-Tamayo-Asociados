...
export function ProfessionalGallery({ posts, compact = false }: { posts: MediaPost[]; compact?: boolean }) {
  ...
  useEffect(() => {
    ...
    let visible = false;
    const observer = new IntersectionObserver(([entry]) => { visible = entry.isIntersecting; }, { threshold: .3 });
  ...
  return <section className="overflow-hidden border-y border-border bg-card px-5 py-20 sm:px-8 sm:py-28 lg:px-12" aria-label="Galería profesional">
    <div className="mx-auto max-w-[1344px]">
      ...
      <div ref={rail} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} onFocusCapture={() => setHover(true)} onBlurCapture={() => setHover(false)} onTouchStart={() => setAuto(false)} className={compact ? 'flex snap-x snap-mandatory gap-5 overflow-x-auto pb-5' : 'grid gap-6 sm:grid-cols-2 lg:grid-cols-3'}>
