...
function MediaDialog({ value, onChange, onSave, onUpload, loading }: { value: MediaPost | null; onChange: (value: MediaPost | null) => void; onSave: (value: MediaPost) => void; onUpload: (file: File, key: 'thumbnailUrl') => void; loading: boolean }) { if (!value) return null; const update = (key: keyof MediaPost, next: string | boolean | number) => onChange({ ...value, [key]: next }); return <Dialog open onOpenChange={(open) => !open && onChange(null)}><DialogContent className="max-h-[92vh] overflow-y-auto bg-white sm:max-w-2xl"><DialogHeader><DialogTitle className="text-2xl">{value.id ? 'Editar publicación' : 'Añadir a la vitrina'}</DialogTitle><DialogDescription>Para la galería de Inicio y Perfil, seleccione Fotografía. Puede cambiar imagen, título, descripción, orden y visibilidad.</DialogDescription></DialogHeader><div className="mt-3 grid gap-5 sm:grid-cols-2"><FormField label="Plataforma"><select value={value.platform} onChange={(e) => update('platform', e.target.value)} className="h-8 rounded-lg border border-input px-2 text-sm"><option>TikTok</option><option>Instagram</option><option>Facebook</option><option>YouTube</option><option>Fotografía</option></select></FormField><FormField label="Categoría"><Input value={value.category} onChange={(e) => update('category', e.target.value)} /></FormField><div className="sm:col-span-2"><FormField label="Título"><Input value={value.title} onChange={(e) => update('title', e.target.value)} /></FormField></div><div className="sm:col-span-2"><FormField label="Enlace de la publicación"><Input type="url" value={value.url} onChange={(e) => update('url', e.target.value)} /></FormField></div><div className="sm:col-span-2"><FormField label="Descripción"><Textarea value={value.caption} onChange={(e) => update('caption', e.target.value)} /></FormField></div><div className="sm:col-span-2"><ImageUploader title="Portada de la publicación" url={value.thumbnailUrl} onFile={(file) => onUpload(file, 'thumbnailUrl')} /></div><FormField label="Orden"><Input type="number" value={value.displayOrder} onChange={(e) => update('displayOrder', Number(e.target.value))} /></FormField><label className="flex items-center justify-between rounded-xl border border-black/10 px-4 py-3 text-sm"><span>Publicado</span><Switch checked={value.active} onCheckedChange={(checked) => update('active', checked)} /></label></div><button onClick={() => onSave(value.platform === 'Fotografía' ? { ...value, url: value.url || value.thumbnailUrl } : value)} disabled={loading || !value.title || (value.platform === 'Fotografía' ? !value.thumbnailUrl : !value.url)} className="mt-5 inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#191713] px-5 text-sm font-semibold text-white disabled:opacity-40"><Save className="size-4" /> Guardar publicación</button></DialogContent></Dialog>; }
